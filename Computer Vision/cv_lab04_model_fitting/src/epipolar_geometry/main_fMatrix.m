...
if (clickPoints)
    ...
    %load('matches_rect1_vs_rect2.mat')
end
...
[e1, e2] = getEpipoles(F)
% show clicked points
figure(1),clf, imshow(img1, []); hold on, plot(x1s(1,:), x1s(2,:), '*r'); plot(e1(1),e1(2), 'ok');
figure(2),clf, imshow(img2, []); hold on, plot(x2s(1,:), x2s(2,:), '*r'); plot(e2(1),e2(2), 'ok');
