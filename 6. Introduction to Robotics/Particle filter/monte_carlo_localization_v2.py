...
class ParticleFilter(object):
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        for p in self.particles:
            ...
            errors.append(self.get_prediction_error_squared(laser_scan))
        ...
        sum_of_exp = sum([exp(-e) for e in errors])
        self.weights[:] = [exp(-e)/sum_of_exp for e in errors]
        for w in self.weights:
            self.sum_of_weights.append(self.sum_of_weights[-1]+w)
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
        ...
        measured_ranges, angles = subsample_laser_scan(laser_scan_msg)
        
        # TODO: simulate a laser scan using one of the methods of this class
        x=particule.x
        y=particule.y
        yaw_in_map= particule.theta
        min_range= self.laser_min_range
        max_range= self.laser_max_range
        sim_ranges = simulate_laser_scan_for_particle(x, y, yaw_in_map, angles, min_range, max_range)
        ...
        for i in range(len(sim_ranges)):
            norm_error += (sim_ranges[i]-measured_ranges[i])**2
        return norm_error
