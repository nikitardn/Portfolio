...
def rrt_search(G, tx, ty):
	...
	while dist_to_goal>SMALLSTEP:
	    p_random = genPoint()
		if visualize: canvas.markit(p_random[0],p_random[1])
	    p_closest  = closestPointToPoint(G,p_random)
	    p_new      =  p_closest + lineFromPoints(p_closest,p_random)*pointPointDistance(p_closest,p_random)
